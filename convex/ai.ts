...
// Interface for Spaced Repetition Items
...
// New function to generate spaced repetition items
export const generateSpacedRepetitionItems = action({
  args: {
    progressData: v.array(
      v.object({
        topic: v.string(),
  ...
  handler: async (ctx, args): Promise<SpacedRepetitionResponse | null> => {
    try {
      const openai = getOpenAIClient();
      const modelConfig = getModelConfig("flashcards", "simple"); // Can reuse flashcard model or define a new task type
      ...
      const systemPrompt = `You are an AI assistant creating spaced repetition learning items for a medical student.
      Analyze the provided progress data and target topics.
      Generate ${args.itemCount || 5} distinct items for review. Each item should have an 'id', 'prompt', 'answer', 'topic', and optionally 'itemType'.
      Prioritize items from topics with lower confidence or those not reviewed recently.
      Return the response as a single JSON object with an "items" array:
      {
        "items": [
          { "id": "sr1", "prompt": "Question or cue...", "answer": "Expected answer...", "topic": "Relevant Topic", "itemType": "recall" }
        ]
      }
      Ensure the output is ONLY the JSON object.`;

      const userPrompt = `My current progress: ${JSON.stringify(formattedProgress)}.
      ${args.targetTopics ? `Focus on these topics: ${args.targetTopics.join(", ")}.` : ""}
      Please generate ${args.itemCount || 5} spaced repetition items.`;
      ...
      const resultText = response.choices[0].message.content;
      if (!resultText) {
          console.error("OpenAI returned empty content for spaced repetition items.");
          return null;
      }
      try {
        const data = JSON.parse(resultText) as SpacedRepetitionResponse;
        if (data && Array.isArray(data.items)) {
            return data;
        } else {
            console.error("Parsed spaced repetition data is not in expected format:", data);
             const jsonMatch = resultText.match(/```json\n([\s\S]*?)\n```/);
            if (jsonMatch && jsonMatch[1]) {
                try {
                    const nestedData = JSON.parse(jsonMatch[1]) as SpacedRepetitionResponse;
                    if (nestedData && Array.isArray(nestedData.items)) return nestedData;
                } catch(e) { /* ignore */ }
            }
            return null;
        }
      } catch (parseError) {
        console.error("Error parsing JSON for spaced repetition items:", parseError, "\nRaw:", resultText);
        return null;
      }
    } catch (error) {
      console.error("Error in generateSpacedRepetitionItems action:", error);
      throw new Error("Failed to generate spaced repetition items: " + String(error));
    }
  },
});
